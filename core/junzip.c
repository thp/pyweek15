...
int jzReadEndRecord(JZFile *zip, JZEndRecord *endRecord) {
    ...
    if((fileSize = zip->tell(zip)) <= JZ_END_RECORD_SIZE) {
        fprintf(stderr, "Too small file to be a zip!");
        return Z_ERRNO;
    }

    if(zip->seek(zip, fileSize - JZ_END_RECORD_SIZE, SEEK_SET)) {
        fprintf(stderr, "Cannot seek in zip file!");
        return Z_ERRNO;
    }

    if(zip->read(zip, endRecord, JZ_END_RECORD_SIZE) < JZ_END_RECORD_SIZE) {
        fprintf(stderr, "Couldn't read end of zip file!");
        return Z_ERRNO;
    }

    if (endRecord->signature != 0x06054B50) {
        fprintf(stderr, "Invalid end record signature!");
        return Z_ERRNO;
    }
    ...
    long centralDirectoryRealOffset = fileSize - JZ_END_RECORD_SIZE - endRecord->centralDirectorySize;
    endRecord->_archiveOffset = (uint32_t)(centralDirectoryRealOffset - endRecord->centralDirectoryOffset);
...
int jzReadCentralDirectory(JZFile *zip, JZEndRecord *endRecord,
    ...
    if(zip->seek(zip, endRecord->centralDirectoryOffset + endRecord->_archiveOffset, SEEK_SET)) {
        fprintf(stderr, "Cannot seek in zip file!");
        return Z_ERRNO;
    }
    ...
    for(i=0; i<endRecord->numEntries; i++) {
        ...
        header.offset = fileHeader.relativeOffsetOflocalHeader + endRecord->_archiveOffset;
...
int jzReadData(JZFile *zip, JZFileHeader *header, void *buffer) {
    ...
    if(header->compressionMethod == 0) { // Store - just read it
        ...
        for(compressedLeft = header->compressedSize,
            ...
            // Read next chunk
            strm.avail_in = (int)zip->read(zip, jzBuffer,
            ...
            strm.avail_out = (int)uncompressedLeft;
            strm.next_out = bytes;
